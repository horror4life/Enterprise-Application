        ...
        try
        {
            ...
            if (outlookRecords.Read())
            {
                ...
                do
                {
                    ...
                    upvotes.Text = outlookRecords["VIDEO_UPVOTES"].ToString();
                    TableCell downvotes = new TableCell();
                    downvotes.Text = outlookRecords["VIDEO_DOWNVOTES"].ToString();
                    TableCell views = new TableCell();
                    views.Text = outlookRecords["VIDEO_VIEWS"].ToString();
                    // Applying the cells to the row
                    ...
                    rating.ColumnSpan = 3;
                    rating.Text = outlookRecords["PERCENTAGE"] + "%";
                    // Applying the cells to the row
